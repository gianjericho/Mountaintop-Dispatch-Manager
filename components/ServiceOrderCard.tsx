...
import { ServiceOrder, AppMode, OrderStatus } from '../lib/supabase/types';
...
export function ServiceOrderCard({
  ...
  return (
    <div className={`bg-slate-800/90 border border-slate-700 rounded-xl p-4 shadow-lg hover:border-slate-600 transition-all text-slate-100 flex flex-col justify-between ${order.is_processed ? 'ring-1 ring-emerald-500/40 bg-emerald-950/10' : ''}`}>
      {/* Top Header Row */}
      ...
      {/* Tech Checklist Section (for Dispatched / Done tabs) */}
      {(isActive || isDone) && (
      ...
      <div className="flex items-center justify-between gap-2 pt-2 border-t border-slate-700/60 mt-auto">
        <span className="text-xs text-slate-500">
          {order.date_reported ? `Rep: ${order.date_reported}` : ''}
          {order.dateAdded ? ` | Add: ${order.dateAdded}` : ''}
          {order.dateDone ? ` | Done: ${order.dateDone}` : ''}
        </span>
